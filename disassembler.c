/*
  Disassembler for 8080 processor. Reads in hex stream and prints out
...
// CODE SOURCE : https://github.com/kpmiller/emulator101/blob/master/8080emu-first50.c
int Disassemble8080Op(unsigned char *codebuffer, int pc)
{
  ...
  switch (*code)
    ...
    default :
      printf("Did not match any of the opcodes\n");
    }
	
  return opbytes;
}

void decode8080Code (unsigned char *buffer, int length) {
  unsigned char *copy = buffer;
  
  int pc = 0;

  // start program counter at 0, and then move program counter the amount
  // that the op code requires it to move to get to the next op code until
  // you get to end of file
  while (pc < length) {
    pc += Disassemble8080Op(copy, pc);
  }
}
...
void decode8080CodeFile(char *filename) {
  ...
  printf ("base memory of buffer : %i\n", (int) (fsize * sizeof(char)));
  printf ("bytes read : %i\n", (int) bytesRead);
  printf ("random byte : %c", buffer[100]);
  */

  decode8080Code(buffer, fsize);
}
